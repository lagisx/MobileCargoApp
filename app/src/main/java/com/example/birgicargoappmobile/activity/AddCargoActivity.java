...
public class AddCargoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            Log.d("ADDCARGO", "Current User ID: " + currentUserId);
            ...
            etPriceWithVat = findViewById(R.id.et_price_with_vat);  // ← НОВОЕ
            ...
            Log.e("ADDCARGO", "Error in onCreate: " + e.getMessage(), e);
    ...
    private void loadUserPhone() {
        ...
        Log.d("ADDCARGO", "Loading user with filter: " + filter);

        usersApi.getUserById(filter).enqueue(new Callback<List<User>>() {
            @Override
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                Log.d("ADDCARGO", "User response code: " + response.code());

                if (response.isSuccessful() && response.body() != null && response.body().size() > 0) {
                    User user = response.body().get(0);
                    userPhone = user.getPhone() != null ? user.getPhone() : "";
                    Log.d("ADDCARGO", "Phone loaded: " + userPhone);
                    Toast.makeText(AddCargoActivity.this, "Профиль загружен: " + userPhone, Toast.LENGTH_SHORT).show();
                } else {
                    Log.e("ADDCARGO", "Failed to load user, code: " + response.code());
                }
            }

            @Override
            public void onFailure(Call<List<User>> call, Throwable t) {
                Log.e("ADDCARGO", "Error loading user: " + t.getMessage());
            }
        });
    }
    ...
    private void addCargo() {
        try {
            ...
            cargo.setPriceWithVat(parseDouble(etPriceWithVat.getText().toString()));  // ← НОВОЕ
            ...
            Log.d("ADDCARGO", "Sending cargo: product=" + cargo.getProduct()
                    + ", from=" + cargo.getFromLocation()
                    + ", to=" + cargo.getToLocation()
                    + ", price=" + cargo.getPriceByCard()
                    + ", priceWithVat=" + cargo.getPriceWithVat()  // ← ЛОГИРОВАНИЕ
                    + ", phone=" + cargo.getContactPhone()
            ...
            cargoApi.addCargo(cargo).enqueue(new Callback<Void>() {
                ...
                public void onResponse(Call<Void> call, Response<Void> response) {
                    ...
                    Log.d("ADDCARGO", "Response code: " + response.code());
                    ...
                    if (response.isSuccessful()) {
                        ...
                        Log.e("ADDCARGO", "Error: " + response.code() + " " + response.message());
                        Toast.makeText(AddCargoActivity.this, "Ошибка: " + response.code(), Toast.LENGTH_SHORT).show();
                    }
                }
                ...
                public void onFailure(Call<Void> call, Throwable t) {
                    btnAddCargo.setEnabled(true);
                    btnAddCargo.setText("Добавить груз");
                    Log.e("ADDCARGO", "Network error: " + t.getMessage(), t);
                    Toast.makeText(AddCargoActivity.this, "Ошибка сети: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });

        } catch (Exception e) {
            Log.e("ADDCARGO", "Exception in addCargo: " + e.getMessage(), e);
    ...
    private double parseDouble(String value) {
        try {
            ...
            Log.e("ADDCARGO", "Parse error: " + e.getMessage());
            return 0;
        }
    }
}
